...
load_args set_load_args(TASK_TYPE task, network *net, char **paths, list *options, int N)
{
    ...
    if(task==SEGMENTATION){
        ...
        if (tag){
            ...
            args.labels = get_labels(label_list);
                        ...
                        int gpu, char *quantized_cfg, char *filename)
{
    ...
    if (filename){
        ...
        if(task==SEGMENTATION){
            ...
            validate_classifier_from_net(*net, 2, args, acc);
        }
...
void analyze_model_ranges(TASK_TYPE task, char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus)
{
    int i;
    srand(time(0));
    int seed = rand();
    ...
    while(get_current_batch(net) < net->max_batches || net->max_batches == 0){
        ...
        analyze_weights_and_activations(net, train, prof);
        free_data(train);
    }
    char buff[256];
